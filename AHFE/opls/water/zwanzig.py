...
# ------------ forward direction --------------------
f1 = np.genfromtxt(sys.argv[1], dtype=np.float128, skip_header=1, usecols=-1, delimiter=' ')
f2 = np.genfromtxt(sys.argv[2], dtype=np.float128, skip_header=1, usecols=-1, delimiter=' ')
frwd = f2 - f1 #np.genfromtxt(sys.argv[1], dtype=None, skip_header=0, delimiter=' ')
